<script lang="ts">
	...
	import { Cloud, Copy, Database, Plug, RefreshCw } from '@lucide/svelte';
	const mcpUrl='https://orbitfsmcp.vercel.app/mcp';
	let runtime=$state<any>(null),loading=$state(true),error=$state(''),copied=$state(false);
	...
	async function copyMcp(){await navigator.clipboard.writeText(mcpUrl);copied=true;setTimeout(()=>copied=false,1600);}
	load(); const addon=$derived(addons.get('mcp'));
</script>
<div class="mx-auto max-w-5xl space-y-5 p-4 md:p-6">
	<div class="flex items-start justify-between gap-3"><div><h1 class="text-xl font-semibold">MCP overview & settings</h1><p class="text-sm text-muted-foreground">Cloud deployment, OAuth, ChatGPT UI and client access.</p></div><Button variant="outline" size="sm" onclick={load} disabled={loading}><RefreshCw class="size-4"/>Refresh</Button></div>
	...
	<Card><CardHeader><CardTitle>ChatGPT App / Plugin</CardTitle><CardDescription>OAuth 2.1 + PKCE and MCP Apps UI are enabled. ChatGPT authenticates through your normal OrbitFS Project account.</CardDescription></CardHeader>
		<CardContent class="space-y-3 text-sm"><div class="flex flex-col gap-2 sm:flex-row sm:items-center"><code class="min-w-0 flex-1 overflow-x-auto rounded-md border bg-muted/40 p-2">{mcpUrl}</code><Button variant="outline" onclick={copyMcp}><Copy class="size-4"/>{copied?'Copied':'Copy MCP URL'}</Button></div>
			<div class="grid gap-2 md:grid-cols-3"><div class="rounded-md border p-3"><strong>1. Add app</strong><p class="mt-1 text-xs text-muted-foreground">In ChatGPT Developer Mode, create an app using the MCP URL above.</p></div><div class="rounded-md border p-3"><strong>2. Sign in</strong><p class="mt-1 text-xs text-muted-foreground">ChatGPT discovers OAuth and sends you to OrbitFS Project to approve access.</p></div><div class="rounded-md border p-3"><strong>3. Use UI</strong><p class="mt-1 text-xs text-muted-foreground">OrbitFS tools and the interactive MCP Apps dashboard become available in ChatGPT.</p></div></div>
			<p class="text-xs text-muted-foreground">OAuth clients are recorded in MCP Client Registry and can be disconnected there.</p></CardContent></Card>
