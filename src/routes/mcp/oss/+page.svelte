...
<div class="mx-auto max-w-5xl space-y-5 p-4 md:p-6">
	...
	{:else}
		...
		<label class="space-y-1 text-sm"><span>Preset button name</span><input class="w-full rounded-md border bg-background p-2" maxlength="40" bind:value={presetMetadata[activePreset].displayName} disabled={!canManagePresetNames()} placeholder={presetMetadata[activePreset].defaultDisplayName} /></label>
				...
				<div class="space-y-2"><p class="text-sm font-medium">Context bundles for this preset</p><div class="grid gap-2 sm:grid-cols-2">{#each bundles as bundle}<label class="flex items-center justify-between gap-2 rounded-md border p-3 text-sm"><span class="min-w-0 break-words">{bundle.name}</span><input type="checkbox" checked={bundleAssignments[activePreset].some((item) => item.bundleId === bundle.id)} onchange={() => toggleBundle(bundle)} disabled={!canManageStartup()} /></label>{/each}</div>{#if bundles.length === 0}<p class="rounded-md border border-dashed p-3 text-sm text-muted-foreground">No enabled context bundles are available.</p>{/if}</div>
				{/if}
			</CardContent></Card>
