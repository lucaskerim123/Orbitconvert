...
export async function indexLibraryItem(user:OrbitUser,workspaceId:string,id:string) {
	...
	const content=(await sourceText(workspaceId,item)).replace(/\r\n?/g,'\n'); const lines=content.split('\n'); const sections:any[]=[]; let current:any={title:item.name,lineStart:1,lines:[]};
	const flush=(lineEnd:number)=>{const body=current.lines.join('\n').trim(); if(body) sections.push({id:uid('sec'),workspaceId,itemId:id,title:current.title,level:1,headingPath:[current.title],lineStart:current.lineStart,lineEnd,preview:body.slice(0,500),wordCount:body.split(/\s+/).filter(Boolean).length,content:body,indexedAt:now()});};
	for(let i=0;i<lines.length;i++){const m=lines[i].match(/^#{1,6}\s+(.+)/);if(m){flush(i);current={title:m[1].trim(),lineStart:i+1,lines:[]};}else current.lines.push(lines[i]);} flush(lines.length);
	state.sections=state.sections.filter((x:any)=>x.itemId!==id).concat(sections); item.sectionCount=sections.length; item.indexedAt=now(); item.sourceTracking={size:Buffer.byteLength(content),signature:crypto.createHash('sha256').update(content).digest('hex')}; state.sourceHistory.push({id:uid('chg'),itemId:id,indexedAt:now(),kind:'indexed',addedSections:sections.length,changedSections:0,removedSections:0}); await saveLibrary(workspaceId,state); return {item,sections,indexed:true};
...
export async function retrieveLibrary(user:OrbitUser,workspaceId:string,input:any) {
	...
	const words=q.split(/\s+/).filter(Boolean); const results=state.sections.filter((s:any)=>!allowed||allowed.has(s.itemId)).map((s:any)=>{const hay=`${s.title} ${s.preview} ${s.content||''}`.toLowerCase();const score=words.reduce((n,w)=>n+(hay.includes(w)?1:0),0);return {...s,score,item:state.items.find((x:any)=>x.id===s.itemId)};}).filter((x:any)=>!words.length||x.score>0).sort((a:any,b:any)=>b.score-a.score).slice(0,limit);
