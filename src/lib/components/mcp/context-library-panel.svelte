<script lang="ts">
  ...
  type Entry = { type: 'file' | 'folder'; path: string; recursive: boolean; required: boolean; priority: number; attachmentType?: 'path' | 'profile'; profileId?: string | null; profileName?: string | null };
  ...
  let workspaces = $state<Workspace[]>([]), workspaceId = $state(''), bundles = $state<Bundle[]>([]), profiles = $state<Profile[]>([]);
  let loading = $state(true), saving = $state(false), error = $state(''), savedMessage = $state(''), editingId = $state<string | null>(null);
  let picker = $state(''), pickerType = $state<'file' | 'folder'>('file'), selectedProfileId = $state('');
  ...
  function hydrateEntry(entry: Entry): Entry {
    if (entry.attachmentType === 'profile') return { ...entry };
  ...
  function reset() {
    editingId = null; picker = ''; selectedProfileId = ''; savedMessage = '';
  ...
  async function loadBundles() {
    ...
    loading = true; error = '';
    try {
      const [bundleData, profileData] = await Promise.all([
...
<div class="mx-auto max-w-5xl space-y-4 p-3 sm:p-4 md:p-6">
  <div><h2 class="flex items-center gap-2 text-lg font-semibold"><BookOpen class="size-5" />Bundle Library</h2><p class="text-sm text-muted-foreground">Saved groups of files, folders and dependent bundles used by startup presets.</p></div>
  ...
  {#if workspaces.length === 0}<Card><CardContent class="p-4 text-sm text-muted-foreground">No workspace with OSS/CCS management permission is available.</CardContent></Card>{:else}
    <Card><CardContent class="p-4"><label class="space-y-1 text-sm"><span>Workspace</span><select class="w-full rounded-md border bg-background p-2" bind:value={workspaceId} onchange={() => { reset(); loadBundles(); }}>{#each workspaces as ws}<option value={ws.id}>{ws.name}</option>{/each}</select></label></CardContent></Card>
